...
def make_Graph(news_df) :
    ...
    news_df['article_date'] = news_df['article_date'].apply(lambda x : str(x[0:4]))

    for i in year :
        temp_df = news_df[news_df.article_date == i]
...
if (saved_file == 'y') :
    file_num = input("몇 개를 파일로 저장하시겠습니까 ?")
    while file_num > 0 :
        num = input("파일로 저장할 분야? 1.IT/과학 2.경제 3.사회 4.생활/문화 5.세계 6.정치 ")
        select_type.append(num)
        file_num -= 1
    make_file(news_df,select_type)    
